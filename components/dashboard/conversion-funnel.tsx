        ...
        {funnelData.map((stage, i) => (
          <div key={stage.stage} className="flex items-center gap-2.5">
            ...
            >
              ...
              >
                <span
                  className={`font-heading text-[10px] font-semibold ${i >= 3 ? "text-white" : "text-indigo-600"}`}
                >
                  {stage.count}
                </span>
              </div>
            </div>
            <span className="w-8 text-right font-heading text-[10px] text-gray-400">
              {stage.pct}%
            </span>
          </div>
        ))}
...
function deriveFunnel(metrics: DashboardMetrics) {
  ...
  return [
    { stage: "Inbound Calls", count: total, pct: 100 },
    {
      stage: "Authenticated",
      count: authenticated,
      pct: Math.round((authenticated / total) * 100),
    },
    {
      stage: "Load Matched",
      count: matched,
    ...
    },
    {
      stage: "Offer Made",
      count: offered,
      pct: Math.round((offered / total) * 100),
    },
    {
      stage: "Booked",
      count: booked,
