  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {loads.map((load) => (
        <Card
          key={load.load_id}
          className={`cursor-pointer p-4 shadow-sm transition-shadow hover:shadow-md ${load.urgency === "critical" ? "border-rose-200" : ""}`}
        ...
        >
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            {load.pitch_count != null && (
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${load.pitch_count > 8 ? "bg-rose-50 text-rose-600" : "bg-gray-100 text-gray-500"}`}>
